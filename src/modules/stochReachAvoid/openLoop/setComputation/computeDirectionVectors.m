...
% hyperplane that of dimension state_dim-2.
...
% Inputs:
% -------
%   no_of_direction_vectors    - Number of unique directions defining the
%                                polytope vertices 
...
%
    ...
    % Check state_dim is a positive scalar
    assert( isscalar(state_dim) &&...
                state_dim > 0, ...
            'SReachTools:invalidArgs', ...
    ...
    assert( rank(affine_hull_of_interest_2D.Ae) == state_dim - 2, ...
            'SReachTools:invalidArgs', ...
            'Dimension of affine hull is not sys.state_dim-2');
    assert( affine_hull_of_interest_2D.Dim == state_dim, ...
            'SReachTools:invalidArgs', ...
...
% [~,~,exit_flag_for_linprog]=linprog(zeros(state_dim,1), ...
%                                     [],[], ...
