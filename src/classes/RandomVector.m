            ...
            switch(lower(rv_type))
                ...
                case 'userdefined'
                    ...
                    if size(X, 2) ~= n_instances
                        throw(SrtInvalidArgsError(sprintf(['Generator ', ...
                            'function did not return the appropriate number',...
                            ' of realizations. Got %d instances, expected ',...
                            '%d instances.'], n_instances, size(X,2))));
                    end
                    % Set the dimension
                    obj.dim = size(X, 1);
