...
%
    ...
    while abs(error_quadrature)>desired_accuracy
        ...
        if points_power > warning_iteration
            warning('SReachTools:runtime', sprintf(['Exceeded %d iterations',...
                '--- Required accuracy: %1.2e | Current error: %1.2e'], ...
                    warning_iteration, desired_accuracy, error_quadrature));
        end        
        points_power=points_power+1;
    end
    if points_power > warning_iteration
        warning('SReachTools:runtime', sprintf('Took %d iterations',...
            points_power-1));
    end
