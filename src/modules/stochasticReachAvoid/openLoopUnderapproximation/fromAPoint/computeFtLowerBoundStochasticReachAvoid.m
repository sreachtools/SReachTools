function [lower_bound_on_stochastic_reach_avoid_problem,...
          optimal_input_vector] =...
                       computeFtLowerBoundStochasticReachAvoid(...
                                                  ...
                                                  desired_accuracy,...
                                                  PSoptions)
% SReach/stochasticReachAvoid/
% computeFtLowerBoundStochasticReachAvoid: Solve the stochastic
% reach-avoid problem (lower bound on the probability and an open-loop
% controller synthesis) using Fourier transform and convex optimization
% (Internal function --- assumes arguments are all ok)
% =============================================================================
%
% computeFtLowerBoundStochasticReachAvoid implements the Fourier
...
% inputs. Please use getFtLowerBoundStochasticReachAvoid instead.
% For the use of user-provided initial guess, see
...
%   H_matrix                    - Concatenated input matrix (see
...
% getFtBasedUnderapproximateStochasticReachAvoidSet and
% reachAvoidProbabilityAssumingValidInitialState
%
% Notes:
% ------
...
%
    ...
    if isempty(guess_optimal_input_vector)
        ...
        assert(exist('cvx_begin','file')==2,...
               'SReach:setup_error',...
               ['This function uses CVX if no initial guess (input policy) ',...
                'for the solver is provided. Please get CVX from ',...
                'http://cvxr.com.']);
        ...
        cvx_begin quiet
            ...
            variable slack_variable(length(concatenated_target_tube_b))
            minimize norm(slack_variable)
            subject to
                resulting_X_for_xmax == mean_X_sans_input + H_matrix * ...
                                            guess_concatentated_input_vector
                concatenated_target_tube_A * resulting_X_for_xmax <= ...
                                            concatenated_target_tube_b +...
                                            slack_variable
                concatenated_input_space_A * guess_concatentated_input_vector...
                                <= concatenated_input_space_b
        cvx_end
    ...
    if all(isnan(guess_concatentated_input_vector))
        ...
        negativeLogReachAvoidProbabilityGivenInputVector =...
          @(input_vector)-log(reachAvoidProbabilityAssumingValidInitialState(...
                                                  input_vector,...
                                                  mean_X_sans_input,...
                                                  covariance_X_sans_input,...
                                                  H_matrix,...
        ...
        % subject to
        %        concatenated_input_space_A * U \leq concatenated_input_space_b
        % Given x_0
        [optimal_input_vector, optimal_negative_log_reach_avoid_probability]=...
...
%discount_vector = 0.1.^[0:length(concatenated_target_tube_b)-1];
...
%    sqrt(diag(concatenated_target_tube_A*concatenated_target_tube_A')); 
%cvx_begin quiet
    %variable guess_concatentated_input_vector(length_input_vector)
    %variable resulting_X_for_xmax(length_state_vector)
    %variable R
    ...
    %subject to
        ...
        %concatenated_target_tube_A * resulting_X_for_xmax...
                        %+ R * dual_norm_of_safe_set_A <= ...
                                    %concatenated_target_tube_b
        %concatenated_input_space_A * ...
         %guess_concatentated_input_vector <= concatenated_input_space_b
        %R >= 0
%cvx_end
