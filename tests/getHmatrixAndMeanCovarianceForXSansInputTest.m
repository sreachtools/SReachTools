    ...
    switch ME.identifier
        case 'SReach:invalidArgs'
            if strcmp(ME.message, ...
                      ['Expected a sys.state_dimension-dimensional',...
...
try
    [H_matrix, mean_X_sans_input, covariance_X_sans_input, Abar, G_matrix] =...
               getHmatMeanCovForXSansInput(sys,...
                                                        initial_state,...
                                                        time_horizon);
    assert(sum(sum(abs(Abar - Abar_saved)))<1e-8,...
           'SReach:internal',...
           'Mismatch in Abar');
    assert(sum(sum(abs(H_matrix - H_matrix_saved)))<1e-8,...
           'SReach:internal',...
           'Mismatch in H_matrix');
    ...
    assert(sum(sum(abs( covariance_X_sans_input - covariance_disturbance * G_matrix_saved * G_matrix_saved')))<1e-8,...
           'SReach:internal',...
           'Incorrect covariance_X_sans_input');
    [H_matrix, mean_X_sans_input, covariance_X_sans_input] =...
               getHmatMeanCovForXSansInput(sys,...
                                                        initial_state,...
                                                        time_horizon);
