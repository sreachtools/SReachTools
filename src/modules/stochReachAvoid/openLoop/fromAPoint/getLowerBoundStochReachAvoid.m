    ...
    if ~target_tube(1).contains(initial_state)
        ...
        switch(lower(method))
            ...
            case 'cccpwl-closed'
                ...
                [lb_stoch_reach_avoid, optimal_input_vector, optimal_input_gain, input_satisfaction_probabilty, risk_alloc] = ...
                ...
                varargout{2} = input_satisfaction_probabilty;
                varargout{3} = risk_alloc;
            case 'cccpwl-closed-cvx'
                switch length(varargin)
                    case 3
                        desired_accuracy = varargin{1};
                        max_state_violation_prob = varargin{2};
        ...
        if lb_stoch_reach_avoid<0
            ...
            if strcmpi(method,'ccciter') || strcmpi(method,'cccpwl')
                ...
                warn_str = ['Methods ''ccciter'' or ''cccpwl'' works only',...
                    ' if the maximal reach probability is above 0.5.'];
            elseif strcmpi(method,'cccpwl-closed-cvx')
%                 warning(sprintf('Method ''cccpwl-closed-cvx'' failed!'));
                warn_str = ('The fixed slack variables (=0) could be causing the infeasibility!');             
            else
                exc = SrtInternalError(['%s resulted in an infeasible ', ...
                    'optimization problem'], method);
                throw(exc);
            end
            warning(sprintf(strcat(method,' returned a trivial lower bound.\n',warn_str)));             
        end
    end
end
