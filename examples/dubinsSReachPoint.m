...
if particle_open_run_nongauss
    ...
    if prob_particle_open_nongauss > 0
        % Optimal mean trajectory construction
        % mean_X = Z * x_0 + H * U + G * \mu_W
        opt_mean_X_particle_open_nongauss =  Z * ...
        init_state_particle_open_nongauss + H * ...
        opt_input_vec_particle_open_nongauss + G * muW_nongauss;
        opt_mean_traj_particle_open_nongauss = ...
            reshape(opt_mean_X_particle_open_nongauss, ...
        ...
        simulated_prob_particle_open_nongauss = sum(mcarlo_result) / ...
            n_mcarlo_sims;
    else
...
if voronoi_open_run_nongauss
    ...
    if prob_voronoi_open_nongauss > 0
        % Optimal mean trajectory construction
        % mean_X = Z * x_0 + H * U + G * \mu_W
        opt_mean_X_voronoi_open_nongauss =  Z * ...
            init_state_voronoi_open_nongauss + H * ...
            opt_input_vec_voronoi_open_nongauss + G * muW_nongauss;
        opt_mean_traj_voronoi_open_nongauss = ...
            reshape(opt_mean_X_voronoi_open_nongauss, ...
                sys_nongauss.state_dim,[]);
        % Check via Monte-Carlo simulation
        ...
        simulated_prob_voronoi_open_nongauss = sum(mcarlo_result) / ...
            n_mcarlo_sims;
    else
...
if voronoi_affine_run_nongauss
    ...
    [prob_voronoi_affine_nongauss, opt_input_vec_voronoi_affine_nongauss, ...
        opt_input_gain_voronoi_affine_nongauss, ...
        kmeans_info_affine_nongauss] = SReachPoint('term', ...
            'voronoi-affine', sys_nongauss, ...
            init_state_voronoi_affine_nongauss,...
            target_tube, opts);
    elapsed_time_voronoi_affine_nongauss = toc(timerVal);
    fprintf('Computation time: %1.3f\n', elapsed_time_voronoi_affine_nongauss);
    if prob_voronoi_affine_nongauss > 0
        % mean_X = Z * x_0 + H * (M \mu_W + d) + G * \mu_W
        opt_mean_X_voronoi_affine_nongauss = Z * ...
            init_state_voronoi_affine_nongauss + H * ...
            opt_input_vec_voronoi_affine_nongauss + ...
            (H * opt_input_gain_voronoi_affine_nongauss + G) * muW_nongauss;
        % Optimal mean trajectory construction
        opt_mean_traj_voronoi_affine_nongauss = reshape( ...
            opt_mean_X_voronoi_affine_nongauss, sys_nongauss.state_dim,[]);
        % Check via Monte-Carlo simulation
        concat_state_realization_voa = generateMonteCarloSims(...
            n_mcarlo_sims_affine, sys_nongauss, ...
            init_state_voronoi_affine_nongauss, ...
        ...
        simulated_prob_voronoi_affine_nongauss = sum(mcarlo_result) / ...
            n_mcarlo_sims_affine;
    else
...
for itt = time_horizon:-1:0
    if itt==0
        % Remember the first the tube
        h_target_tube = plot(target_tube_cell{1}, 'LineStyle', '--', ...
            'alpha', 0.2, 'color', 'y');
    else
        plot(target_tube_cell{itt+1},'alpha', 0.2, 'LineStyle', '--', ...
            'color', 'y');        
    end            
end
axis equal        
...
if particle_open_run_nongauss
    if prob_particle_open_nongauss > 0
        h_opt_mean_particle_nongauss = scatter(...
            ...
            30, 'r^', 'filled','MarkerEdgeColor','k', 'DisplayName', ...
            'Mean trajectory (particle-open)');  
        legend_cell{end+1} = 'Mean trajectory (particle-open)';    
        h_vec(end+1) = h_opt_mean_particle_nongauss;        
        polytopesFromMonteCarloSims(...
            concat_state_realization_pao(sys_nongauss.state_dim+1:end,:), ...
                sys_nongauss.state_dim, [1,2], ...
                {'color', 'r', 'edgecolor', 'r', 'linewidth', 2, 'alpha', ...
                    0.15, 'LineStyle', ':'});
    end
    disp('>>> SReachPoint with particle-open')
    fprintf('SReachPoint approx. prob: %1.2f | Simulated prob: %1.2f\n',...
...
if voronoi_open_run_nongauss 
    if prob_voronoi_open_nongauss > 0
        h_opt_mean_voronoi_nongauss = scatter(...
            ...
            30, 'cv', 'filled','MarkerEdgeColor','k', 'DisplayName', ...
            'Mean trajectory (voronoi-open)');  
        legend_cell{end+1} = 'Mean trajectory (voronoi-open)';    
        h_vec(end+1) = h_opt_mean_voronoi_nongauss;
        polytopesFromMonteCarloSims(...
            concat_state_realization_voo(sys_nongauss.state_dim+1:end,:), ...
                sys_nongauss.state_dim, [1,2], {'color', 'c', 'edgecolor', ...
                    'c', 'linewidth', 2, 'alpha', 0.15, 'LineStyle', ':'});
    end
    disp('>>> SReachPoint with voronoi-open')
    fprintf('SReachPoint approx. prob: %1.2f | Simulated prob: %1.2f\n',...
        prob_voronoi_open_nongauss, simulated_prob_voronoi_open_nongauss);
...
if voronoi_affine_run_nongauss
    if prob_voronoi_affine_nongauss > 0
        h_opt_mean_voronoi_affine_nongauss = scatter(...
            [init_state_voronoi_affine_nongauss(1), ...
                opt_mean_traj_voronoi_affine_nongauss(1,:)], ...
            [init_state_voronoi_affine_nongauss(2), ...
                opt_mean_traj_voronoi_affine_nongauss(2,:)], ...
            30, 'bs', 'filled','DisplayName', ...
            'Mean trajectory (voronoi-affine)');
        legend_cell{end+1} = 'Mean trajectory (voronoi-affine)';
        h_vec(end+1) = h_opt_mean_voronoi_affine_nongauss;
        polytopesFromMonteCarloSims( ...
            concat_state_realization_voa(sys_nongauss.state_dim+1:end,:), ...
                sys_nongauss.state_dim, [1,2], {'color', 'b', 'edgecolor', ...
                    'b', 'linewidth', 2, 'alpha', 0.15, 'LineStyle', ':'});
    end
