...
%
    ...
    if options.verbose >= 1
        if ~strcmpi(xmax_soln.cvx_status, 'Solved') 
            fprintf(['Non-trivial Wmax computation failed in CVX: %s\n'], ...
                xmax_soln.cvx_status);
        else
            if xmax_soln.reach_prob < prob_thresh
                fprintf(['No polytope exists | Maximum reach probability: ', ...
                    '%1.4f < %1.4f (level of the stochastic reach set)'], ...
                    xmax_soln.reach_prob, prob_thresh);
            elseif isempty(options.set_of_dir_vecs)
                fprintf(['No polytope exists | Maximum reach probability: ', ...
                    '%1.4f >= %1.4f (level of the stochastic reach set), ', ...
                    'but no direction vector provided'], ...
                    xmax_soln.reach_prob, prob_thresh);
            else
                fprintf(['Polytope exists | Maximum reach probability: ', ...
                    '%1.4f >= %1.4f (level of the stochastic reach set)'], ...
                    xmax_soln.reach_prob, prob_thresh);
            end
        end
    ...
    if ~strcmpi(xmax_soln.cvx_status, 'Solved') || ...
            xmax_soln.reach_prob < prob_thresh || ...
            isempty(options.set_of_dir_vecs)
        ...
        if options.init_safe_set_affine.Dim > 0
            ...
            xmaxPlusdirs = options.set_of_dir_vecs + xmax_soln.xmax;
...
function otherInputHandling(method_str, sys, options)
    ...
    if ~isempty(options.set_of_dir_vecs)
        % Make sure the user specified set_of_dir_vecs is valid
        if size(options.set_of_dir_vecs, 1) ~= sys.state_dim
            throwAsCaller(SrtInvalidArgsError(sprintf(['set_of_dir_vecs ', ...
                'should be a collection of %d-dimensional column vectors.'], ...
                sys.state_dim)));
        end
        ...
        warning('SReachTools:runtime', sprintf(['Empty ', ...
            'options.set_of_dir_vecs means SReachSetCcO will only compute', ...
            ' Wmax.\nWmax is the maximum optimal safety probability over ', ...
...
function [polytope, extra_info] = computePolytopeFromAnchor(x_anchor, sys, ...
    ...
    if sys.input_dim > 0
        ...
        concat_input_space_A = zeros(1, 0);
        concat_input_space_b = zeros(0, 1);
    end
        ...
        switch cvx_status
            case 'Solved'
                ...
                if options.verbose == 2
                    fprintf([' | Sign of the dir. vector: %s | Optimal',...
                        ' theta value (extension along the dir. vector',...
                        '): %1.2f'], num2str(sign(direction')), ...
                        opt_theta_i(direction_index));
                end
            otherwise
